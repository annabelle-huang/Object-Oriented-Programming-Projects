...
from customexceptions import OverdrawError, TransactionSequenceError
...
class Transaction:
    ...
    def check_balance(self, balance):
        ...
        if (self._amt >= 0 or balance >= abs(self._amt)) == False:
            raise OverdrawError()
        else:
            return self._amt >= 0 or balance >= abs(self._amt)
        
    def check_order(self, transactions):
        "Checks whether the input transaction follows chronological order"
        if len(transactions) == 0:
            return True
        for x in transactions:
            if (x._date > self._date):
                # raise error if not in chronological order
                raise TransactionSequenceError(max(transactions).date)
        return True
